...
1. **Start the server:**
```bash
cd /Users/mohityadav/Desktop/Consuma
source venv/bin/activate
python -m uvicorn app.main:app --port 8000
```

2. ** Swagger docs/Postman:** Open http://localhost:8000/docs in browser

3. **Demo sync endpoint** (in new terminal):
```bash
curl -X POST http://localhost:8000/sync \
  -H "Content-Type: application/json" \
  -d '{"data": "hello world", "iterations": 1000}' | python3 -m json.tool
```

4. **Demo async with callback** (start callback server first):
```bash
# Terminal 1: Start callback receiver
python callback_server.py
...
curl -X POST http://localhost:8000/async \
  -H "Content-Type: application/json" \
  -d '{"data": "test", "iterations": 500, "callback_url": "http://localhost:8001/callback"}' | python3 -m json.tool

# Show callback received
curl http://localhost:8001/callbacks | python3 -m json.tool
```

5. **Show request tracking:**
```bash
curl http://localhost:8000/requests?limit=5 | python3 -m json.tool
```

6. **Run load test:**
```bash
APP_CALLBACK_BLOCK_PRIVATE_IPS=false python -m uvicorn app.main:app --port 8000  # restart server
python load_generator.py -n 50 -c 10 --mode both
```
...
- "Used GPT for initial scaffolding"
- "Iteratively refined with help of AI"
...
# Contribution
|---------------------|---------------|
| **Architecture decision**: FastAPI + async workers | main.py, callback_worker.py |
| **SSRF protection**: Block private IPs | config.py `is_private_ip()` |
| **Retry logic**: Exponential backoff | callback_worker.py `_deliver_callback()` |
| **Testing strategy**: Unit tests + load tests | test_api.py, load_generator.py |
| **Debugging**: SSRF blocking localhost issue | Show the warning logs |


> 1. **Architecture**: Chose FastAPI with an async worker pool pattern for scalability
> 2. **Security**: Implemented SSRF protection to prevent callback URL abuse
> 3. **Reliability**: Added exponential backoff with jitter for callback retries
> 4. **Testing**: Created both unit tests and a load generator with p50/p95/p99 stats
> 5. **Debugging**: Identified and fixed issues like SSRF blocking local testing"
